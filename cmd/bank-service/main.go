...
func main() {
	...
	if err := db.AutoMigrate(&bank.Bank{}); err != nil {
	...
	grpcSvc := bank.NewGRPCServer(svc)
	pb.RegisterBankServiceServer(grpcServer, grpcSvc)

	// Initialize HTTP server
	app := fiber.New()

	// Register HTTP handlers
	handler := bank.NewHandler(svc)
